...
targetCipher = target.decode("hex")
...
for i in range(0, len(targetCipher)):
    if i in keyIndex:
        decryptedTarget = decryptedTarget + chr(ord(targetCipher[i]) ^ keyIndex[i])
    else:
        decryptedTarget = decryptedTarget + chr(0)

decryptedMessages = []

for i in range(0, len(cipherTexts)):
    message = ""
    for j in range(0, len(cipherTexts[i])):
        if j in keyIndex:
